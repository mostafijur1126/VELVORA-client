...
const WomenLeggingsPage = async () => {
  const category = ["Leggings", "Tights"];
    ...
    <div className="w-full mx-auto px-4 md:px-6 lg:px-8 py-20">
      <div>
        <h1 className="text-4xl font-semibold pb-6">
          Women - Leggings (<span>{products.length}</span>)
        </h1>
      </div>
      {products.length > 0 ? (
        ...
        <div>
          <h1>No Products Available Now</h1>
        </div>
      )}
    </div>
  );
...
export default WomenLeggingsPage;
